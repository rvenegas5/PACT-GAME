...
const ModalCard = ({setOpenCard, currentCard,setCurrentCard}) => {
    ...
    return (
        <> 
            <Modal open={setOpenCard} onClose={handleClose}> 
                <Box sx={style}>
                    ...
                    <>
                        <>
                            {typeCardKnow ?
                                <h5 className="message-modal">Carta de Conocimiento <QuestionAnswerTwoToneIcon/></h5>
                                :
                                <h5 className="message-modal">Carta de Situación </h5>}
                                
                            <Divider sx={{ bgcolor: "secondary.light", margin: "8px" }} flexItem />
                                ...
                                {Object.keys(currentCard.options).map((key, i) => (
                                    <>
                                        <div className="option">
                                            <button className='border-radius: 50%;' key={i}
                                                onClick={() => handleWait(key)}>{key}
                                            </button>
            ...
            {openFeed  && <ModalFeed setOpenFeed={setOpenFeed} 
